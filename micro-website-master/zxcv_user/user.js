...
$(function(){
    ...
    function userNameFormatter(value, row, index){
        var html = "<a href='#' class='edit_user' onclick='showUser(" + row.id + ", \"" + row.newsNo + "\")' >" + row.title + "</a> ";
        return html;
    }
...
function editUser(userId, userNo){
    ...
    /*if(dataArr[0].Attribute==1){
        $("#editUserForm input[name=Attribute]:eq(0)").prop("checked",true);
        $("#editUserForm input[name=Attribute]:eq(1)").prop("checked",false);
    }
    else if(dataArr[0].Attribute==2){
        $("#editUserForm input[name=Attribute]:eq(1)").prop("checked",true);
        $("#editUserForm input[name=Attribute]:eq(0)").prop("checked",false);
    }*/
    //先清空角色复选框
    // $('#editUserForm .edit input').prop('checked',false);
